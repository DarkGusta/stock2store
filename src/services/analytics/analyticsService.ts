...
export const getAnalyticsData = async (): Promise<AnalyticsData> => {
  try {
    ...
    const { count: productsSoldCount, error: productsSoldError } =
      await supabase
        .from("items")
    ...
    console.log("Total Users:", userCountData?.total_users);

    if (userCountError) {
      console.error("Error fetching active users:", userCountError);
    }

    const activeUsersCount = userCountData?.total_users || 0;
    ...
    const recentActivity = [
      ...
      ...(recentTransactions?.map((transaction) => ({
        event:
          transaction.notes || transaction.transaction_type === "sale"
            ? "Order processed"
            : transaction.transaction_type === "status_change"
            ? "Inventory updated"
            : "Low stock alert",
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      const monthlyItems =
        itemsWithRevenue?.filter((item) => {
          const itemDate = new Date(item.updated_at);
          return itemDate >= monthDate && itemDate < nextMonth;
        }) || [];

      const monthRevenue = monthlyItems.reduce(
